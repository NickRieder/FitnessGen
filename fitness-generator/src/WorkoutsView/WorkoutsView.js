            ...
            .then((event) => {
              switch (lowerBody[i]) {
                case 'Quadriceps':
                  ...
                  let firstQuad = Math.floor(Math.random() * Object.keys(event).length);
                  let secondQuad = Math.floor(Math.random() * Object.keys(event).length);

                  //create object of exercises in order of days
                  const quadObject = { "0": event[firstQuad], "1": event[secondQuad] };
                  setQuads(quadObject);
                  break;
                case 'Hamstrings':
  ...
  return (
    <>
      <div className="workout_view">
        ...
        {isFiveDay ? <FiveDay user={user} quads={quads} hamstrings={hamstrings} glutes={glutes} calves={calves} back={back} chest={chest} biceps={biceps} tricep={tricep} shoulder={shoulder} core={core} mobility={mobilityWorkout} /> : ""}
      </div>
    </>
  )
}
